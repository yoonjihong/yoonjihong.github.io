...
일반적으로 리액트에서 컴포넌트는 부모 컴포넌트 리렌더링 되거나 Props 또는 상태 값이 변경됐을 때 리렌더링된다.

<br />

`또한 리액트는 Props와 상태 변경을 감지를 위해 이전 값과 현재 값을 하나씩 비교하지 않고 성능 최적화를 위해 얕은 비교를 실행하기 때문에 리액트에서 불변변성을 지키는게 중요하다.`
...
값을 변경 했지만 리액트에선 얕은 비교를 실행하기 때문에 값이 변경되었다고 판단하지 않는다.
