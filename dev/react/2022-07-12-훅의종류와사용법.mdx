...
> setState를 사용하지 않고 직접적으로 값을 변경하게되면 리액트는 값 변경을 감지하지 못한다.  
> 또한 리액트는 setState를 실행했을때 이전 값과 비교 후 렌더링 여부를 판단하기 때문에 직접적으로 값을 변경해선 안된다.
...
`updater에 인자값은 항상 최신 값으로 유지`되기 때문에 최신 값이 필요하거나 순차 실행이 필요할 땐 setState의 updater함수를 전달해서 사용한다.
